...
def timer_callback():
    ...
    if l_black and c_black and r_black:
        if state == 'FOLLOWING':
            ...
            junction_count += 1
            node.get_logger().info(f"Junction {junction_count} detected!")
    ...
    if all_white and junction_count >= TARGET_JUNCTION and not action_done:
        ...
        node.get_logger().info("target zone! Place cube")
        # Call the imported action
        ...
        if success:
            ...
            return # Exit callback to keep robot stopped
        else:
            return 
    ...
    if state == 'FOLLOWING':
        ...
        if max(l_val, c_val, r_val) > BLACK_THRESHOLD:
            ...
            message.linear.x = 0.05 # Stop if line lost
...
def main(args=None):
    ...
    mechanism = PickPlaceMechanism(node.getlogger())
